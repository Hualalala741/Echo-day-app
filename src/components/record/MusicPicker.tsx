  ...
  return (
    <div className="w-full rounded-xl border border-border bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        <Music2 className="h-4 w-4 shrink-0 text-brand" />
        <span className="text-xs font-semibold text-muted-foreground">Today&apos;s Song</span>
      </div>

      {!musicSearchQuery?.trim()|| loading ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-border bg-muted/50 p-6">
          <div className="flex items-end gap-[3px] h-7">
            {[0, 1, 2, 3, 4].map((i) => (
              <span
                key={i}
                className="w-[3px] rounded-full bg-brand origin-bottom"
                style={{
                  height: "100%",
          ...
          <span className="text-sm text-muted-foreground">Searching for songs…</span>
        </div>
      ) : error ? (
        ...
        <div className="space-y-4">
          {musicReason && (
            <p className="px-0.5 text-sm italic leading-relaxed text-muted-foreground">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex gap-2">
              <button
                ...
                disabled={loading}
                className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-border bg-card px-3 text-xs font-medium text-muted-foreground hover:bg-muted disabled:pointer-events-none disabled:opacity-40"
              >
                <RefreshCw className="h-3.5 w-3.5" />
                More
              </button>
              <button
                ...
                className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-border bg-card px-3 text-xs font-medium text-muted-foreground hover:bg-muted disabled:opacity-40"
              >
            ...
            <button
              ...
              disabled={saving}
              className="inline-flex h-11 shrink-0 w-[20%] items-center justify-center gap-2 rounded-xl bg-brand px-5 text-sm font-semibold text-brand-foreground shadow-md transition hover:brightness-105 active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-55"
            >
              {saving ? (
